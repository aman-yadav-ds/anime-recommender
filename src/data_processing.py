...
class DataProcessor:
    def __init__(self, output_dir: str, input_dataframes: Optional[Dict[str, pd.DataFrame]] = None):
        """
        ...
        """
        self.input_dataframes = input_dataframes or {}
    ...
    def _optimize_dtypes(self, df: pd.DataFrame) -> pd.DataFrame:
        """This code is generated by claude to optimize the df for memory efficiency since it's huge."""
        for col in df.columns:
            ...
            if df[col].dtype in ['int64', 'int32']:
                if df[col].min() >= 0:
                    if df[col].max() < 65536:
                        df.loc[:,col] = df[col].astype('uint16')
                    elif df[col].max() < 4294967296:
                        df.loc[:,col] = df[col].astype('uint32')
                else:
                    if df[col].min() >= -32768 and df[col].max() < 32768:
    ...
    def load_data(self, usecols: List, input_file: Optional[str] = None) -> pd.DataFrame:
        ...
        """
        try:
            if 'animelist.csv' in self.input_dataframes:
                logger.info("Using passed animelist DataFrame from input_dataframes dict.")
                self.rating_df = self.input_dataframes['animelist.csv'][usecols]
            ...
            self.rating_df = self._optimize_dtypes(self.rating_df)
    ...
    def process_anime_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            # Doing the same thing we did with load_data
            if 'anime.csv' in self.input_dataframes:
                df = self.input_dataframes['anime.csv'].copy()
                logger.info("Using pre-loaded anime DataFrame")
            else:
                df = pd.read_csv(ANIME_CSV)
                logger.info(f"Loaded anime data from file: {ANIME_CSV}")
            
            synopsis_cols = ["MAL_ID", "Name", "Genres", "sypnopsis"]
            if 'anime_synopsis.csv' in self.input_dataframes:
                synopsis_df = self.input_dataframes['anime_synopsis.csv'][synopsis_cols].copy()
            else:
